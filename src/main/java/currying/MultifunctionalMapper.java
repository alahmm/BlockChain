...
class MultifunctionalMapper {
    ...
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>>
            MULTIFUNCTIONAL_MAPPER = lstFunc->
            lstVals -> lstVals.stream()
                    .map(curValue -> lstFunc.stream().
                            reduce(IntUnaryOperator::andThen).orElse(x -> x).applyAsInt(curValue))
    ...
    public static final UnaryOperator<List<Integer>> MULT_TWO_AND_THEN_ADD_ONE_TRANSFORMATION =
            list -> list.stream().map(l -> l * 2 + 1).collect(Collectors.toList());
    ...
    public static final UnaryOperator<List<Integer>> SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION =
            list -> list.stream().map(l -> l * l ).toList().stream().map(m -> (m+1) % 2 == 0 ? m + 1 : m + 2).toList();
